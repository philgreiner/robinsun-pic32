...
int setInterruptMask(uint8_t mask, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    ...
    MyDelayMs(POZYX_DELAY_REMOTE_WRITE);
  }
...
int setUWBSettings(UWB_settings_t UWB_settings, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    ...
    MyDelayMs(2 * POZYX_DELAY_LOCAL_WRITE);
    ...
    MyDelayMs(2 * POZYX_DELAY_REMOTE_WRITE);
    if (status == POZYX_FAILURE){
      return status;
    }
...
int setSensorMode(uint8_t sensor_mode, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    ...
    // MyDelayMs required to switch modes.
    MyDelayMs(20);
  }
  else{
...
int doPositioning(coordinates_t *position, uint8_t dimension, int32_t height, uint8_t algorithm)
{
...
int getPositioningAnchorIds(uint16_t anchors[], int anchor_num, uint16_t remote_id)
{
  ...
  if(remote_id == NULL){
    getDeviceListSize(&device_list_size, NULL);
    ...
    MyDelayMs(POZYX_DELAY_REMOTE_FUNCTION);
    if (anchor_num < device_list_size){
      return POZYX_FAILURE;
    }
...
void __attribute__((weak)) __assert (const char *func, const char *file, int line, const char *failedexpr)
{
  ...
  if (Serial){
    ...
    // platform independent MyDelayMs to allow the string to be printed
    MyDelayMs(10);
  }
