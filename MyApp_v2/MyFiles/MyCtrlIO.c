...
void MyDataUpdate(void)
{
    ...
    dataIN.r_wheel_speed = (double) dirR*(speedR1*256.0 + speedR2)*2*M_PI/532.42; // in rad/s 
    ...
    dataIN.l_wheel_speed = (double) dirL*(speedL1*256.0 + speedL2)*2*M_PI/472.83; 
}
...
void MyCtrlUpdate(void)
{
    ...
    // Integrate the error
    erreurIntR += (omegaref[R_ID]-rspeed)*0.001*5; //T_DISCRETISATION;
    erreurIntL += (omegaref[L_ID]-lspeed)*0.001*5; //*T_DISCRETISATION;
