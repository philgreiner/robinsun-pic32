...
//JDL    SD_WE_TRIS = INPUT;            //Write Protect - input
...
#ifdef __PIC32MX__
    BYTE   clear;
//JDL    putcSPI((BYTE)data_out);
//JDL    clear = getcSPI();
    SpiChnPutC(SPI_CHANNEL1A, (BYTE)data_out);  //JDL
    clear = SpiChnGetC(SPI_CHANNEL1A);          //JDL
...
//JDL    putcSPI((BYTE)0xFF);
//JDL    return (BYTE)getcSPI();
    SpiChnPutC(SPI_CHANNEL1A, (BYTE)0xFF);  //JDL
    return (BYTE)SpiChnGetC(SPI_CHANNEL1A); //JDL
...
//JDL   SPICLOCK = 0;
//JDL   SPIOUT = 0;                  // define SDO1 as output (master or slave)
//JDL   SPIIN = 1;                  // define SDI1 as input (master or slave)
...
void InitSPISlowMode(void)
{
    #if defined __C30__ || defined __C32__
    	#ifdef __PIC32MX__
            // JDL OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2);
            // JDL SPIBRG = SPICalutateBRG(GetPeripheralClock(), 400000);
    
            SPIENABLE = 0;  //JDL
            SpiChnConfigure(SPI_CHANNEL1A, SPI_OPEN_MSTEN | SPI_OPEN_SMP_END | SPI_OPEN_CKE_REV | SPI_CONFIG_MODE8);    //JDL
            SPIBRG = SPICalutateBRG(GetPeripheralClock(), 400000);  //JDL
            SPIENABLE = 1;  //JDL
