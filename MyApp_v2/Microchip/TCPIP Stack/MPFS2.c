...
// Begin JDL
#include "MyCamera.h"
WORD        MyCamera_Image_fatID = -1;
MPFS_HANDLE MyCamera_Image_hMPFS = MAX_MPFS_HANDLES;
// End JDL
	...
	for(i = 0; i < numFiles; i++)
	{
		...
		if(hashCache[i&0x07] == nameHash)
		{
			...
			{// Filename matches, so return true
                ...
                // Begin JDL
                if ((strcmp(cFile,"images/Logoepl.jpg") == 0) && (MyCamera_Image_Length > 0))
                    // If no image has been captured : MyCamera_Image_Length = 0
                {
                    MPFSStubs[hMPFS].addr       = (MPFS_PTR) MyCamera_Image_Buffer;
                    MPFSStubs[hMPFS].bytesRem   = (DWORD) MyCamera_Image_Length;
                    MyCamera_Image_fatID        = i;
                    MyCamera_Image_hMPFS        = hMPFS;
                }
                // End JDL
...
MPFS_HANDLE MPFSOpenID(WORD hFatID)
{
    ...
    // Begin JDL
    if ((hFatID == MyCamera_Image_fatID) && (MyCamera_Image_Length > 0)) {
        MPFSStubs[hMPFS].addr       = (MPFS_PTR) MyCamera_Image_Buffer;
        MPFSStubs[hMPFS].bytesRem   = (DWORD) MyCamera_Image_Length;
        MyCamera_Image_hMPFS        = hMPFS;
    }
...
void MPFSClose(MPFS_HANDLE hMPFS)
{
    ...
    // Begin JDL
    if (MyCamera_Image_hMPFS == hMPFS) {
        MyCamera_Image_fatID = -1;
        MyCamera_Image_hMPFS = MAX_MPFS_HANDLES;
    }
    // End JDL
}
...
BOOL MPFSGet(MPFS_HANDLE hMPFS, BYTE* c)
{
    ...
    // Read function for EEPROM
		...
		*c = XEERead();
        ...
        else *c = *((BYTE *) MPFSStubs[hMPFS].addr);
        // End JDL
        MPFSStubs[hMPFS].addr++;
	#else
		#if defined(__C30__)
...
{	
	...
	// Read the data
        ...
        else memcpy(cData, (BYTE *) MPFSStubs[hMPFS].addr, wLen);
        // End JDL
        MPFSStubs[hMPFS].addr += wLen;
		MPFSStubs[hMPFS].bytesRem -= wLen;
	#else
...
static void _LoadFATRecord(WORD fatID)
{
    ...
    // Begin JDL
    if ((fatID == MyCamera_Image_fatID) && (MyCamera_Image_Length > 0)) {
        fatCache.data = (DWORD) MyCamera_Image_Buffer;
        fatCache.len  = (DWORD) MyCamera_Image_Length;
    }
    // End JDL
}
