...
void game_map_drawing(CtrlStruct *cvs)
{
	...
	for (x = 0; x <= 42; x++)
	{
		...
		// *** 0/ borders
		 ...
		 // *** 1/ Block bar ***
			else if (x <= (6+radius_robot) ) { // Close to violet cabin
	    ...
	    // *** 2/ central bar vertical ***
			else if (x >= (18 - radius_robot) && x <= (16 + radius_robot) ) {
				if (y >= (21 - radius_robot) && y <= (42 + radius_robot)) {
						cvs->param->game_map[x][y] = 0;
				}
				else cvs->param->game_map[x][y] = 1;
	   ...
	   // *** 4/ Lower and Upper right corner steps ***
				...
				else if (y <= (5 + radius_robot)) {		 // lower right corner
					cvs->param->game_map[x][y] = 0;
				}// end else if y
				else cvs->param->game_map[x][y] = 1;
