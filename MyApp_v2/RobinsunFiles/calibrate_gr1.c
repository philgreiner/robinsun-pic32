...
void calibrate_start(CtrlStruct * cvs)
{
	...
	case CALIBRATEY:
		...
		{
			#ifdef SIMU_PROJECT
				cvs->state->position[1] = (cvs->inputs->robot_id == ROBOT_B || cvs->inputs->robot_id == ROBOT_R) ? -1.44 : 1.44;
				cvs->state->position[2] = (cvs->inputs->robot_id == ROBOT_B || cvs->inputs->robot_id == ROBOT_R) ? M_PI_2 : -M_PI_2;
			#else
				cvs->state->position[1] = -1.44;
	...
	case GOTOY:
		...
		//#ifdef SIMU_PROJECT
		//	printf("Calibrate : Y = %f\n",cvs->state->position[1]);
		//#endif

		if (fabs(cvs->state->position[1]) - 1.1 > 0.01)
		{
			cvs->state->omegaref[R_ID] = 2 * M_PI;
			cvs->state->omegaref[L_ID] = 2 * M_PI;
		}
	...
	case TURN:
		if (M_PI - fabs(cvs->state->position[2]) > 0.015)
		{
			#ifdef SIMU_PROJECT
				...
				//printf("theta error: %f\n", M_PI - fabs(cvs->state->position[2]));
			#else
				cvs->state->omegaref[R_ID] = 2 * M_PI;
				cvs->state->omegaref[L_ID] = -2 * M_PI;
			#endif
	...
	case GOHOMETURN:
		if (fabs(M_PI_2 - fabs(cvs->state->position[2])) > 0.015)
		{
#ifdef SIMU_PROJECT
			cvs->state->omegaref[R_ID] = (cvs->inputs->robot_id == ROBOT_B || cvs->inputs->robot_id == ROBOT_R) ? -2 * M_PI : 2 * M_PI;
			cvs->state->omegaref[L_ID] = (cvs->inputs->robot_id == ROBOT_B || cvs->inputs->robot_id == ROBOT_R) ? 2 * M_PI : -2 * M_PI;
			//printf("theta error: %f\n", fabs(M_PI_2 - fabs(cvs->state->position[2])));
#else
			cvs->state->omegaref[R_ID] = -2 * M_PI;
			cvs->state->omegaref[L_ID] = 2 * M_PI;
#endif
