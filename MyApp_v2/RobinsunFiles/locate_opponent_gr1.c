...
void locate_opponent(CtrlStruct *cvs) {
    ...
    double opponent_radius = 0.00;
    double opponents_xpos[4] = {-42.0, -42.0, -42.0, -42.0};
    ...
    for (i = 0; i < 6; i = i + 3) { // for : i=0 --> front side / i=3 --> rear side
        if (dist[i] < max_dist) { // left
            if (dist[i + 1] < max_dist) { // left and center
                if (dist[i + 2] < max_dist) { // left, center and right --> opponent in front of the center sonar
                ...
                else { // left and center only --> opponent in front of the left sonar
            ...
            else if (dist[i + 2] < max_dist) { // left and right --> 2 opponents : one on the left of the left sonar, 
            ...
            else { // left only --> opponent on the left of the left sonar
        ...
        else if (dist[i + 2] < max_dist) { // right
            if (dist[i + 1] < max_dist) { // right and center
        ...
        else if (dist[i + 1] < max_dist) { // center only --> opponent in front of the center sonar
